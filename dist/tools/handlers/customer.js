...
function buildQBAddress(input) {
    ...
    if (input.line4)
        addr.Line4 = input.line4;
    if (input.line5)
        addr.Line5 = input.line5;
    if (input.city)
        addr.City = input.city;
    if (input.country_sub_division_code)
        addr.CountrySubDivisionCode = input.country_sub_division_code;
    if (input.postal_code)
        addr.PostalCode = input.postal_code;
    if (input.country)
        addr.Country = input.country;
    if (input.lat)
        addr.Lat = input.lat;
    if (input.long)
        addr.Long = input.long;
    return addr;
}
function formatAddress(addr, label) {
    if (!addr)
        return [`${label}: (none)`];
    const parts = [];
...
export async function handleCreateCustomer(client, args) {
    const { display_name, given_name, middle_name, family_name, suffix, company_name, email, phone, mobile, bill_address, ship_address, notes, taxable, parent_ref, job, bill_with_parent, preferred_delivery_method, sales_term_ref, draft = true, } = args;
    // Build QB Customer object
    const customerObj = {
        DisplayName: display_name,
    };
    if (given_name)
        customerObj.GivenName = given_name;
    if (middle_name)
        customerObj.MiddleName = middle_name;
    if (family_name)
        customerObj.FamilyName = family_name;
    if (suffix)
        customerObj.Suffix = suffix;
    if (company_name)
        customerObj.CompanyName = company_name;
    if (email)
        customerObj.PrimaryEmailAddr = { Address: email };
    if (phone)
        customerObj.PrimaryPhone = { FreeFormNumber: phone };
    if (mobile)
        customerObj.Mobile = { FreeFormNumber: mobile };
    if (bill_address)
        customerObj.BillAddr = buildQBAddress(bill_address);
    if (ship_address)
        customerObj.ShipAddr = buildQBAddress(ship_address);
    if (notes)
        customerObj.Notes = notes;
    if (taxable !== undefined)
        customerObj.Taxable = taxable;
    if (job !== undefined)
        customerObj.Job = job;
    if (bill_with_parent !== undefined)
        customerObj.BillWithParent = bill_with_parent;
    if (preferred_delivery_method)
        customerObj.PreferredDeliveryMethod = preferred_delivery_method;
    ...
    let salesTermName;
    if (sales_term_ref) {
        const terms = await promisify((cb) => client.findTerms(cb));
        const termList = terms.QueryResponse?.Term || [];
        const match = termList.find(t => t.Name.toLowerCase() === sales_term_ref.toLowerCase() ||
            t.Id === sales_term_ref);
        if (!match) {
    ...
    const result = await promisify((cb) => client.createCustomer(customerObj, cb));
...
export async function handleGetCustomer(client, args) {
    const { id } = args;
    const customer = await promisify((cb) => client.getCustomer(id, cb));
...
export async function handleEditCustomer(client, args) {
    const { id, display_name, given_name, middle_name, family_name, suffix, company_name, email, phone, mobile, bill_address, ship_address, notes, taxable, active, parent_ref, job, bill_with_parent, preferred_delivery_method, sales_term_ref, draft = true, } = args;
    // Fetch current customer
    const current = await promisify((cb) => client.getCustomer(id, cb));
    // Build sparse update
    const updated = {
        Id: current.Id,
    ...
    if (given_name !== undefined)
        updated.GivenName = given_name;
    if (middle_name !== undefined)
        updated.MiddleName = middle_name;
    if (family_name !== undefined)
        updated.FamilyName = family_name;
    if (suffix !== undefined)
        updated.Suffix = suffix;
    if (company_name !== undefined)
        updated.CompanyName = company_name;
    if (email !== undefined)
        updated.PrimaryEmailAddr = { Address: email };
    if (phone !== undefined)
        updated.PrimaryPhone = { FreeFormNumber: phone };
    if (mobile !== undefined)
        updated.Mobile = { FreeFormNumber: mobile };
    if (bill_address !== undefined)
        updated.BillAddr = buildQBAddress(bill_address);
    if (ship_address !== undefined)
        updated.ShipAddr = buildQBAddress(ship_address);
    if (notes !== undefined)
        updated.Notes = notes;
    if (taxable !== undefined)
    ...
    if (job !== undefined)
        updated.Job = job;
    if (bill_with_parent !== undefined)
        updated.BillWithParent = bill_with_parent;
    if (preferred_delivery_method !== undefined)
        updated.PreferredDeliveryMethod = preferred_delivery_method;
    ...
    if (draft) {
        ...
        if (display_name !== undefined)
            previewLines.push(`  Display Name: ${current.DisplayName} → ${display_name}`);
        if (given_name !== undefined)
            previewLines.push(`  Given Name: ${current.GivenName || "(none)"} → ${given_name}`);
        if (middle_name !== undefined)
            previewLines.push(`  Middle Name: ${current.MiddleName || "(none)"} → ${middle_name}`);
        if (family_name !== undefined)
            previewLines.push(`  Family Name: ${current.FamilyName || "(none)"} → ${family_name}`);
        if (suffix !== undefined)
            previewLines.push(`  Suffix: ${current.Suffix || "(none)"} → ${suffix}`);
        if (company_name !== undefined)
            previewLines.push(`  Company: ${current.CompanyName || "(none)"} → ${company_name}`);
        if (email !== undefined)
            previewLines.push(`  Email: ${current.PrimaryEmailAddr?.Address || "(none)"} → ${email}`);
        if (phone !== undefined)
            previewLines.push(`  Phone: ${current.PrimaryPhone?.FreeFormNumber || "(none)"} → ${phone}`);
        if (mobile !== undefined)
            previewLines.push(`  Mobile: ${current.Mobile?.FreeFormNumber || "(none)"} → ${mobile}`);
        if (bill_address !== undefined)
            previewLines.push("  Billing Address: (updating)");
        if (ship_address !== undefined)
            previewLines.push("  Shipping Address: (updating)");
        if (notes !== undefined)
            previewLines.push(`  Notes: ${current.Notes || "(none)"} → ${notes}`);
        if (taxable !== undefined)
            previewLines.push(`  Taxable: ${current.Taxable ?? "(default)"} → ${taxable}`);
        if (active !== undefined)
            previewLines.push(`  Active: ${current.Active !== false} → ${active}`);
        if (parent_ref !== undefined) {
            const newParent = updated.ParentRef?.name || parent_ref;
            previewLines.push(`  Parent: ${current.ParentRef?.name || '(none)'} → ${newParent}`);
        }
        if (job !== undefined)
            previewLines.push(`  Job: ${current.Job ?? false} → ${job}`);
        if (bill_with_parent !== undefined)
            previewLines.push(`  Bill With Parent: ${current.BillWithParent ?? false} → ${bill_with_parent}`);
        if (preferred_delivery_method !== undefined)
            previewLines.push(`  Preferred Delivery: ${current.PreferredDeliveryMethod || '(none)'} → ${preferred_delivery_method}`);
        if (sales_term_ref !== undefined) {
            const newTerm = updated.SalesTermRef?.name || sales_term_ref;
    ...
    const result = await promisify((cb) => client.updateCustomer(updated, cb));
    return {
        content: [{
                type: "text",
