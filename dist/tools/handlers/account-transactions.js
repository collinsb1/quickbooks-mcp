...
import { resolveAccount, getDepartmentCache, getAccountCache, } from "../../client/index.js";
...
export async function handleQueryAccountTransactions(client, args) {
    ...
    if (department) {
        ...
        if (!deptMatch) {
            deptMatch = deptCache.items.find(d => d.FullyQualifiedName?.toLowerCase().includes(department.toLowerCase()));
        }
        if (deptMatch) {
    ...
    const queryResults = await Promise.all(entityTypes.map(async ({ type, finder }) => {
        const pagination = {
        ...
        try {
            ...
            return { type, entities: result.entities };
        }
        catch {
    ...
    for (const { type, entities } of queryResults) {
        const lines = extractAccountLines(entities, type, resolvedAccount.Id, accountCache, resolvedDepartmentId);
        allLines.push(...lines);
    }
    ...
    const totalDebitsCents = sumCents(matchingLines.filter(l => l.amount > 0).map(l => toCents(l.amount)));
    const totalCreditsCents = sumCents(matchingLines.filter(l => l.amount < 0).map(l => toCents(Math.abs(l.amount))));
    ...
    if (truncated) {
        ...
        for (const [key, value] of Object.entries(groupedByTransaction)) {
            if (truncatedTxnKeys.has(key))
                outputGrouped[key] = value;
        }
    }
    else {
