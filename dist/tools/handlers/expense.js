...
export async function handleCreateExpense(client, args) {
    const { payment_type, payment_account, txn_date, entity_name, entity_id, department_name, department_id, class_name, memo, doc_number, lines, draft = true, } = args;
    ...
    if (entityInput) {
        ...
        if (byId) {
            ...
            if (byName) {
                ...
                const byPartial = vendorCacheData.items.find(v => v.DisplayName.toLowerCase().includes(entityInput.toLowerCase()));
                if (byPartial) {
    ...
    // Create the expense
    const result = await promisify((cb) => client.createPurchase(purchaseObject, cb));
...
export async function handleGetExpense(client, args) {
    const { id } = args;
    const expense = await promisify((cb) => client.getPurchase(id, cb));
...
export async function handleEditExpense(client, args) {
    ...
    const current = await promisify((cb) => client.getPurchase(id, cb));
    ...
    if (payment_account !== undefined) {
        ...
        if (!match)
            match = acctCache.byName.get(payment_account.toLowerCase());
        if (!match)
            match = acctCache.items.find(a => a.FullyQualifiedName?.toLowerCase().includes(payment_account.toLowerCase()));
        if (!match)
            throw new Error(`Payment account not found: "${payment_account}"`);
    ...
    if (draft) {
        ...
        if (payment_account !== undefined) {
            const newAcct = updated.AccountRef?.name || payment_account;
        ...
        if (entityInput) {
            const newEntity = updated.EntityRef?.name || entityInput;
    ...
    const result = await promisify((cb) => client.updatePurchase(updated, cb));
    return {
