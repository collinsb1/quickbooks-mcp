...
import { promisify, getDepartmentCache, getClassCache, resolveItem, resolveCustomer, } from "../../client/index.js";
import { validateAmount, toDollars, formatDollars, sumCents, outputReport } from "../../utils/index.js";
export async function handleCreateInvoice(client, args) {
    const { txn_date, customer_name, customer_id, due_date, department_name, department_id, class_name, memo, customer_memo, bill_email, sales_term_ref, allow_online_credit_card_payment, allow_online_ach_payment, doc_number, lines, draft = true, } = args;
    ...
    if (draft) {
        const preview = [
            ...
            "",
            "Lines:",
            ...resolvedLines.map(l => `  ${l.itemRef.name}: Qty ${l.qty} × $${l.unitPriceDollars.toFixed(2)} = $${l.amountDollars.toFixed(2)}${l.description ? ` "${l.description}"` : ""}`),
            "",
    ...
    // Create the invoice
    const result = await promisify((cb) => client.createInvoice(invObject, cb));
...
export async function handleGetInvoice(client, args) {
    const { id } = args;
    const invoice = await promisify((cb) => client.getInvoice(id, cb));
...
export async function handleEditInvoice(client, args) {
    const { id, txn_date, due_date, memo, customer_memo, bill_email, sales_term_ref, allow_online_credit_card_payment, allow_online_ach_payment, customer_name, department_name, class_name, lines: lineChanges, draft = true, } = args;
    // Fetch current Invoice
    const current = await promisify((cb) => client.getInvoice(id, cb));
    ...
    if (customer_memo !== undefined)
        updated.CustomerMemo = { value: customer_memo };
    if (bill_email !== undefined)
        updated.BillEmail = { Address: bill_email };
    if (allow_online_credit_card_payment !== undefined)
        updated.AllowOnlineCreditCardPayment = allow_online_credit_card_payment;
    if (allow_online_ach_payment !== undefined)
        updated.AllowOnlineACHPayment = allow_online_ach_payment;
    ...
    if (draft) {
        ...
        if (customer_memo !== undefined)
            previewLines.push(`  Customer Memo: ${current.CustomerMemo?.value || '(none)'} → ${customer_memo}`);
        if (bill_email !== undefined)
            previewLines.push(`  Bill Email: ${current.BillEmail?.Address || '(none)'} → ${bill_email}`);
        if (sales_term_ref !== undefined) {
            const newTerm = updated.SalesTermRef?.name || sales_term_ref;
            previewLines.push(`  Terms: ${current.SalesTermRef?.name || '(none)'} → ${newTerm}`);
        }
        if (allow_online_credit_card_payment !== undefined)
            previewLines.push(`  Online CC Payment: ${current.AllowOnlineCreditCardPayment ?? '(not set)'} → ${allow_online_credit_card_payment}`);
        if (allow_online_ach_payment !== undefined)
            previewLines.push(`  Online ACH Payment: ${current.AllowOnlineACHPayment ?? '(not set)'} → ${allow_online_ach_payment}`);
        if (customer_name !== undefined) {
            const newCust = updated.CustomerRef?.name || customer_name;
    ...
    const result = await promisify((cb) => client.updateInvoice(updated, cb));
    return {
