...
import { promisify, getAccountCache, getDepartmentCache, getClassCache, resolveItem, resolveCustomer, } from "../../client/index.js";
import { validateAmount, toDollars, formatDollars, sumCents, outputReport } from "../../utils/index.js";
export async function handleCreateSalesReceipt(client, args) {
    const { txn_date, customer_name, customer_id, deposit_to_account, department_name, department_id, class_name, memo, doc_number, lines, draft = true, } = args;
    ...
    if (deposit_to_account) {
        ...
        if (!match)
            match = acctCache.byName.get(deposit_to_account.toLowerCase());
        if (!match)
            match = acctCache.items.find(a => a.FullyQualifiedName?.toLowerCase().includes(deposit_to_account.toLowerCase()));
        if (!match)
            throw new Error(`Deposit account not found: "${deposit_to_account}"`);
    ...
    const result = await promisify((cb) => client.createSalesReceipt(srObject, cb));
...
export async function handleGetSalesReceipt(client, args) {
    const { id } = args;
    const salesReceipt = await promisify((cb) => client.getSalesReceipt(id, cb));
...
export async function handleEditSalesReceipt(client, args) {
    ...
    const current = await promisify((cb) => client.getSalesReceipt(id, cb));
    ...
    const result = await promisify((cb) => client.updateSalesReceipt(updated, cb));
    return {
        content: [{ type: "text", text: `Sales Receipt ${id} updated successfully.\nNew SyncToken: ${result.SyncToken}\nView in QuickBooks: ${qboUrl}` }],
    };
}
//# sourceMappingURL=sales-receipt.js.map