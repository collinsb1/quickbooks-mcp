...
export function getCompanyIdValue() {
    return companyId;
}
...
export async function getClient() {
    ...
    if (!isConfigured) {
        if (isLocalMode()) {
            throw new Error("QuickBooks credentials not configured. Run the qbo_authenticate tool to set up OAuth.");
        }
        else {
            throw new Error("QuickBooks credentials not found in AWS. Check your AWS Secrets Manager and SSM Parameter Store configuration.");
        }
    }
    ...
    qbo = new QuickBooks(credentials.client_id, credentials.client_secret, credentials.access_token, false, // No OAuth 1.0 token secret for OAuth 2.0
    companyId, useSandbox, // Use sandbox if QBO_SANDBOX=true
    ...
    try {
        const tokenInfo = await promisify((cb) => qbo.refreshAccessToken(cb));
    ...
    qbo = new QuickBooks(credentials.client_id, credentials.client_secret, credentials.access_token, false, companyId, useSandbox, false, // Debug mode off
    null, "2.0", credentials.refresh_token);
    return qbo;
}
