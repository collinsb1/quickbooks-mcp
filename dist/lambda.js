...
const remoteToolDefinitions = toolDefinitions.filter((t) => t.name !== "qbo_authenticate");
...
function createServer() {
    const server = new Server({ name: "quickbooks-mcp", version: "1.0.0" }, { capabilities: { tools: {} } });
 ...
 */
function getPublicUrl(event, overridePath) {
 ...
 */
function toWebRequest(event, method) {
 ...
 */
async function toGatewayResult(response) {
 ...
 */
function oauthServerMetadata(event) {
    const metadata = {
 ...
 */
function resourceMetadataResponse(event) {
 ...
 */
function unauthorized(event, description) {
    const resourceUrl = getPublicUrl(event);
    return {
        statusCode: 401,
        headers: {
